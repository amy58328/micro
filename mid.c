...
int main()
{
	...
	while(1)
	{
		...
		for(t=0;t<16;t++)
		{
			test = ((test >> 1) & 0x7fff) | ((test << 15) & 0x8000);
			GPIO_PTB_GPIO = test ;
			delay1(600000);

		}

		int n1[2],n2[2],result[4];

		int v0 = input%16;
		int v1 = input/16;
		int res = v0 * v1 * abs(v0-v1);
		...
		for(i=7;i>=4;i--)
		{
			...
			while(t--)
			{
				if(n1[1] != 0)
				{
					GPIO_PTD_GPIO = index_7LED_NUM[n1[1]];
					GPIO_PTA_GPIO = Digit_4;
					delay1(1000);
				}
				

				GPIO_PTD_GPIO = index_7LED_NUM[n1[0]];
				GPIO_PTA_GPIO = Digit_3;
				delay1(1000);

				if(n2[1] != 0)
				{
					GPIO_PTD_GPIO = index_7LED_NUM[n2[1]];
					GPIO_PTA_GPIO = Digit_2;
					delay1(1000);
				}


				GPIO_PTD_GPIO = index_7LED_NUM[n2[0]];
				GPIO_PTA_GPIO = Digit_1;
				delay1(1000);
				...
				if(result[3] != 0 && i <=4)
				{
					GPIO_PTD_GPIO = index_7LED_NUM[result[3]];
					GPIO_PTA_GPIO = index_7LED[i+3];
					delay1(1000);
					zero = 1;
				}
				
				if((result[2] != 0 || zero != 1) && i <= 5)
				{
					GPIO_PTD_GPIO = index_7LED_NUM[result[2]];
					GPIO_PTA_GPIO = index_7LED[i+2];
					delay1(1000);
					zero = 1;
				}
				
				if((result[1] != 0 || zero != 1) && i<=6)
				{
					GPIO_PTD_GPIO = index_7LED_NUM[result[1]];
				...
				GPIO_PTD_GPIO = index_7LED_NUM[result[0]];
		...
		while(same == input)
		{
			...
			{
				GPIO_PTD_GPIO = index_7LED_NUM[result[3]];
				GPIO_PTA_GPIO = index_7LED[7];
				delay1(1000);
				zero = 1;
			}
			
			if(result[2] != 0 || zero != 1) 
			{
				GPIO_PTD_GPIO = index_7LED_NUM[result[2]];
				GPIO_PTA_GPIO = index_7LED[6];
				delay1(1000);
				zero = 1;
			}
			
			if(result[1] != 0 || zero != 1)
			{
				GPIO_PTD_GPIO = index_7LED_NUM[result[1]];
				GPIO_PTA_GPIO = index_7LED[5];
				delay1(1000);
				zero = 1;
			}

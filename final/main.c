...
int number[1][8] = {{4,0,6,2,6,2,3,3}};
int i, j, myi;
int key = 0xFF,flag = 1,tmp,col;
int cursor = 1;
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(a == 0)// left to right
	{
		for (i = 0; i < 8; i++)
		{
			...
			while(myi--)
			{
				for(j=0 ; j<= i ; j++) 
				{
					GPIO_PTD_GPIO = index_7LED_NUM[number[0][7-j]];
					GPIO_PTA_GPIO = index_7LED[7-i+j];
					delay1(1000);
				}
			}
		}
	...
	else if(a == 1)// right to left
	{
		for (i = 0; i < 8; i++)
		{
			...
			while(myi--)
			{
				for(j=0 ; j<= i ; j++) 
				{
					GPIO_PTD_GPIO = index_7LED_NUM[number[0][j]];
...
void keep_light()
{
	...
	while(t--)
	{
		for( i=0 ; i < 8 ; i++)
		{
			GPIO_PTD_GPIO = index_7LED_NUM[number[0][7-i]];
...
{
	int WordValue = 38;
  char ABC[]="New Taipei City, 24205, Taiwan, R.O.C.";
  char i=0;
...
{
	int WordValue = 38;
  char ABC[]="Fu Jen Catholic University, CSIE Dept.";
  char i=0;
...
{
	int WordValue =36;
  char ABC[]="Wish a New and Happy Wonderful Year!";
  char i=0;
...
{
	int WordValue = 37;
  char ABC[]="2019 Good Luck! Good Luck! Good Luck!";
  char i=0;
